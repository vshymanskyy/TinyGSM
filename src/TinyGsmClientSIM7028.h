...
class TinyGsmSim7028 : public TinyGsmSim70xx<TinyGsmSim7028>,
                       public TinyGsmTCP<TinyGsmSim7028, TINY_GSM_MUX_COUNT>,
                       public TinyGsmSMS<TinyGsmSim7028>,
                       public TinyGsmTime<TinyGsmSim7028>,
                       public TinyGsmNTP<TinyGsmSim7028>,
                       public TinyGsmGSMLocation<TinyGsmSim7028>,
                       public TinyGsmBattery<TinyGsmSim7028> {
  friend class TinyGsmSim70xx<TinyGsmSim7028>;
  ...
  friend class TinyGsmSMS<TinyGsmSim7028>;
  friend class TinyGsmGSMLocation<TinyGsmSim7028>;
  friend class TinyGsmGPS<TinyGsmSim7028>;
  friend class TinyGsmTime<TinyGsmSim7028>;
  friend class TinyGsmNTP<TinyGsmSim7028>;
  friend class TinyGsmBattery<TinyGsmSim7028>;
 ...
 public:
  class GsmClientSim7028 : public GsmClient {
   ...
   public:
    ...
    explicit GsmClientSim7028(TinyGsmSim7028& modem, uint8_t mux = 0) {
      init(&modem, mux);
    }
   ...
   */
 public:
  explicit TinyGsmSim7028(Stream& stream)
      : TinyGsmSim70xx<TinyGsmSim7028>(stream) {
 ...
 protected:
  bool initImpl(const char* pin = nullptr) {
    ...
    // Configure unsolicited network registration messages
    // sendAT(GF("AT+CEREG=1"));
...
public:
  bool setBand(const char *band) {
    sendAT(GF("+QCBAND=0,"), band);  // Set the band dynamically
 ...
 protected:
  bool gprsConnectImpl(const char* apn, const char* user = nullptr,
    ...
    // Optional: handle user and pass if needed
    if (user && pwd && strlen(user) > 0 && strlen(pwd) > 0) {
        // Set authentication (if required by the network)
        sendAT(GF("AT+CGAUTH=1,1,\""), user, GF("\",\""), pwd, GF("\""));
        if (waitResponse(10000L) != 1) {
            return false;
        }
    }
